-- INSTALLATION:
-- --------
-- 1. Install HUNIT, ignore the terminal warnings:
--      cabal install HUnit
-- 2. Compile this file (in same folder as HW1.hs):
--      ghc --make -v Spec.hs -o Spec -package HUnit
-- 3. run this file:
--      ./spec
-- 4. If you are struggling with installation or have any questions,
--    please ask ChatGPT or friends instead of messaging me.
--      Chag Sameach!
--
-- NOTE: You may need to disable your antivirus / tell it to trust PATH/TO/FILE/Spec.exe in order to run it.
-- NOTE: if you modify the Spec file, you must recompile it (Step 2) before rerunning!
-- NOTE: if you modify your HW1 file, you must recompile it (ghc -Wall -Werror HW1.hs) before rerunning!
-- NOTE: It's recommended to always rebuild HW1.hs and Spec.hs before rerunning; using ChatGPT you can easily make a
--        .bat (Windows) or .sh (Linux/Mac) file which does this for you!
--
-- PARTIAL RUN:
-- --------
-- If you'd like to run only some tests instead of all of them, for example if you'd like to run only the first 5 tests:
-- 1. Comment out all other tests
-- 2. Remove the comma from the end of the last test
