...
void worker_checker(int worker_id, int num_of_workers, const char *filename, int pipe_write_fd) {
	...
	while ((read = getline(&line, &len, toplist_file)) != -1) {
		...
		// dividing the work (line reading) between the workers
		if (line_number % num_of_workers == worker_id) {
			...
			if (URL_UNKNOWN == (res = check_url(line))) {
				...
				// exiting child, parent processes
				kill_val = kill(0, SIGKILL);
				if (kill_val == -1) {
					perror("didn't not manage to exit the process");
				}
	...
	if(results.amount > 0){
		// writing the results to the parent process via the pipe
		if (write(pipe_write_fd, &results, sizeof(ResultStruct)) == -1) {
			perror("unable to write to pipfd");
			exit(EXIT_FAILURE);
		}
		if (close(pipe_write_fd) == -1) {
			perror("unable to close to pipfd");
			exit(EXIT_FAILURE);
		}
	}
...
void parallel_checker(int num_of_processes, const char *filename) {
	...
	for (worker_id = 0; worker_id  < num_of_processes; ++worker_id ) {
		...
		if (PID < 0) {
			...
			close(pipefd[0]); // nor using read end
	...
	// TODO
	int staus = 0 ;
	while(wait(&staus) > 0); // waiting for all child processes
//  wait(NULL);	

	for (worker_id = 0; worker_id  < num_of_processes; ++worker_id ) {

		// TODO - sum the results
		close(pipefd[1]); // not using write end
