...
public class Parser {
    ...
    public String commandType(){
        ...
        if (this.instruction.indexOf("pop") != -1){
            ...
            return C_PUSH;
        } else if (Arrays.asList(ar).contains(this.instruction)){
            return C_ARITHMETIC;
     ...
     */
    public String arg1(){
        int space = this.instruction.indexOf(" ");
        String command = this.commandType();

        if (command.equals(C_ARITHMETIC)){
            return this.instruction;
        } else {
            return this.instruction.substring(space + 1, this.instruction.indexOf(" ",space + 1));
        }
    }
     ...
     */
    public int arg2(){
        int space = this.instruction.lastIndexOf(" ");
        return Integer.parseInt(this.instruction.substring(space+1));
    }
